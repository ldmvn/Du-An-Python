...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (emailInput && getOtpBtn) {
        ...
        getOtpBtn.addEventListener('click', function() {
            ...
            if (!isValidEmail(email)) {
                if (window.LDMToast) {
                    window.LDMToast.show('Vui lòng nhập email hợp lệ!', 'error');
                }
                return;
            }
            ...
            .then(data => {
                if (data.status === 'success') {
                    if (LDMToast) {
                        LDMToast.show('Đã gửi mã OTP về email của bạn!', 'success');
                    }
                    // Ẩn message cũ
                    if (otpMessage) {
                    ...
                    if (LDMToast) {
                        LDMToast.show(data.message || 'Gửi OTP thất bại!', 'error');
                    }
                    getOtpBtn.textContent = 'Lấy mã';
                    updateOtpButtonState();
                }
            ...
            .catch(error => {
                if (LDMToast) {
                    LDMToast.show('Lỗi kết nối server!', 'error');
                }
                getOtpBtn.textContent = 'Lấy mã';
                updateOtpButtonState();
            });
