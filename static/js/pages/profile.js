...
function copyVoucher(btn, code) {
    navigator.clipboard.writeText(code).then(function () {
        ...
        if (window.LDMToast) LDMToast.show('Đã sao chép mã ' + code, 'success');
        setTimeout(function () {
            btn.innerHTML = orig;
...
function deleteAddress(id) {
    if (typeof LDMConfirm !== 'undefined' && LDMConfirm && LDMConfirm.show) {
        LDMConfirm.show('Bạn có chắc muốn xóa địa chỉ này?', function () {
            doDeleteAddress(id);
        });
    } else {
 ...
 */
function LDMLoadRefundData() {
    // Tải đơn cần hoàn tiền
    fetch('/api/refund-pending/', {
        method: 'GET',
        headers: { 'X-CSRFToken': LDMGetCookie('csrftoken') }
    })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            var listEl = document.getElementById('ldm-refund-pending-list');
            if (data.orders && data.orders.length > 0) {
                var html = '<table class="ldm-refund-table" style="width:100%;font-size:13px;border-collapse:collapse;">';
                ...
                data.orders.forEach(function (o) {
                    ...
                    html += '<td style="padding:8px;text-align:center;"><span class="ldm-ot-badge cancelled">Chờ hoàn tiền</span></td>';
                    html += '<td style="padding:8px;text-align:center;"><button onclick="LDMShowRefundDetail(\'' + o.order_code + '\')" style="padding:4px 12px;background:#3b82f6;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Xem</button></td>';
                    html += '</tr>';
                });
                html += '</tbody></table>';
        ...
        .catch(function (err) {
            document.getElementById('ldm-refund-pending-list').innerHTML = '<p style="color:#ef4444;font-size:14px;text-align:center;padding:20px;">Lỗi tải dữ liệu</p>';
        });
    ...
    fetch('/api/refund-history/', {
        ...
        .then(function (r) { return r.json(); })
        .then(function (data) {
            var listEl = document.getElementById('ldm-refund-history-list');
            if (data.orders && data.orders.length > 0) {
                var html = '';
                data.orders.forEach(function (o) {
                    var productName = o.items.length > 0 ? o.items[0].product_name : '—';
                    html += '<div style="padding:12px;background:#f8fafc;border-radius:8px;margin-bottom:8px;cursor:pointer;" onclick="LDMShowRefundDetail(\'' + o.order_code + '\')">';
                    html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
                    html += '<span style="color:#3b82f6;font-weight:600;font-size:13px;">' + o.order_code + '</span>';
                    html += '<span style="color:#10b981;font-weight:600;font-size:14px;">Đã hoàn ' + LDMFmtVND(o.total_amount) + '</span>';
                    html += '</div>';
        ...
        .catch(function (err) {
            document.getElementById('ldm-refund-history-list').innerHTML = '<p style="color:#ef4444;font-size:14px;text-align:center;padding:20px;">Lỗi tải dữ liệu</p>';
        });
}
 ...
 */
function LDMShowRefundDetail(orderCode) {
    fetch('/api/refund-detail/' + orderCode + '/', {
        ...
        .then(function (r) { return r.json(); })
        .then(function (data) {
            var modal = document.getElementById('LDMRefundDetailModal');
            var body = document.getElementById('LDMRefundDetailBody');
            ...
            o.items.forEach(function (item) {
                ...
                html += '<td style="padding:8px;text-align:right;color:#1e293b;font-weight:600;">' + LDMFmtVND(item.price) + '</td>';
            ...
            html += '<span style="font-size:18px;font-weight:700;color:#ef4444;">' + LDMFmtVND(o.total_amount) + '</span>';
            html += '</div>';

            body.innerHTML = html;
        })
        .catch(function (err) {
            document.getElementById('LDMRefundDetailBody').innerHTML = '<p style="color:#ef4444;text-align:center;">Lỗi tải dữ liệu</p>';
        });
}
 ...
 */
function LDMCloseRefundDetail() {
    document.getElementById('LDMRefundDetailModal').classList.remove('active');
}
 ...
 */
function LDMCloseRefundDetailOnClickOutside(event) {
    if (event.target.classList.contains('ldm-modal-overlay')) {
        LDMCloseRefundDetail();
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    // ===== Chuyển đổi tab =====
    var tabs = document.querySelectorAll('.ldm-pf-tab[data-tab]');
    var panels = document.querySelectorAll('.ldm-pf-tab-panel');

    function activateTab(tabName) {
        tabs.forEach(function (t) { t.classList.remove('active'); });
        panels.forEach(function (p) { p.classList.remove('active'); });
        var targetTab = document.querySelector('.ldm-pf-tab[data-tab="' + tabName + '"]');
    ...
    var PW_PER_PAGE = 4;
    var pwItems = document.querySelectorAll('.ldm-pf-pw-item[data-pw-index]');
    ...
    if (pwItems.length > 0 && pwPagination) {
        ...
        function renderPwPagination(current) {
            ...
            var html = '';
            html += '<button class="ldm-pf-pw-page-btn" data-pw-page="' + (current - 1) + '"' + (current === 1 ? ' disabled' : '') + '><i class="ri-arrow-left-s-line"></i></button>';
            for (var p = 1; p <= totalPages; p++) {
                html += '<button class="ldm-pf-pw-page-btn' + (p === current ? ' active' : '') + '" data-pw-page="' + p + '">' + p + '</button>';
            }
            html += '<button class="ldm-pf-pw-page-btn" data-pw-page="' + (current + 1) + '"' + (current === totalPages ? ' disabled' : '') + '><i class="ri-arrow-right-s-line"></i></button>';
            pwPagination.innerHTML = html;

            pwPagination.querySelectorAll('.ldm-pf-pw-page-btn').forEach(function (btn) {
    ...
    // ===== Phân trang địa chỉ =====
    var ADDR_PER_PAGE = 3;
    var addrItems = document.querySelectorAll('.ldm-pf-addr-card[data-addr-index]');
    ...
    if (addrItems.length > 0 && addrPagination) {
        ...
        function renderAddrPagination(current) {
            ...
            var html = '';
            html += '<button class="ldm-pf-pw-page-btn" data-addr-page="' + (current - 1) + '"' + (current === 1 ? ' disabled' : '') + '><i class="ri-arrow-left-s-line"></i></button>';
            for (var p = 1; p <= addrTotalPages; p++) {
                html += '<button class="ldm-pf-pw-page-btn' + (p === current ? ' active' : '') + '" data-addr-page="' + p + '">' + p + '</button>';
            }
            html += '<button class="ldm-pf-pw-page-btn" data-addr-page="' + (current + 1) + '"' + (current === addrTotalPages ? ' disabled' : '') + '><i class="ri-arrow-right-s-line"></i></button>';
            addrPagination.innerHTML = html;

            addrPagination.querySelectorAll('.ldm-pf-pw-page-btn').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    if (this.disabled) return;
    ...
    var couponItems = document.querySelectorAll('.ldm-pf-coupon-card[data-coupon-index]');
    ...
    if (couponItems.length > 0 && couponPagination) {
        ...
        function renderCouponPagination(current) {
            ...
            var html = '';
            html += '<button class="ldm-pf-pw-page-btn" data-coupon-page="' + (current - 1) + '"' + (current === 1 ? ' disabled' : '') + '><i class="ri-arrow-left-s-line"></i></button>';
            for (var p = 1; p <= couponTotalPages; p++) {
                html += '<button class="ldm-pf-pw-page-btn' + (p === current ? ' active' : '') + '" data-coupon-page="' + p + '">' + p + '</button>';
            }
            html += '<button class="ldm-pf-pw-page-btn" data-coupon-page="' + (current + 1) + '"' + (current === couponTotalPages ? ' disabled' : '') + '><i class="ri-arrow-right-s-line"></i></button>';
            couponPagination.innerHTML = html;

            couponPagination.querySelectorAll('.ldm-pf-pw-page-btn').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    if (this.disabled) return;
    ...
    var orderItems = document.querySelectorAll('.ldm-pf-order-card[data-order-index]');
    ...
    if (orderItems.length > 0 && orderPagination) {
        ...
        function renderOrderPagination(current) {
            ...
            var html = '';
            html += '<button class="ldm-pf-pw-page-btn" data-order-page="' + (current - 1) + '"' + (current === 1 ? ' disabled' : '') + '><i class="ri-arrow-left-s-line"></i></button>';
            for (var p = 1; p <= orderTotalPages; p++) {
                html += '<button class="ldm-pf-pw-page-btn' + (p === current ? ' active' : '') + '" data-order-page="' + p + '">' + p + '</button>';
            }
            html += '<button class="ldm-pf-pw-page-btn" data-order-page="' + (current + 1) + '"' + (current === orderTotalPages ? ' disabled' : '') + '><i class="ri-arrow-right-s-line"></i></button>';
            orderPagination.innerHTML = html;

            orderPagination.querySelectorAll('.ldm-pf-pw-page-btn').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    if (this.disabled) return;
    ...
    var allTabs = document.querySelectorAll('.ldm-pf-tab[data-tab]');
    allTabs.forEach(function (tab) {
        tab.addEventListener('click', function () {
            var target = this.getAttribute('data-tab');
            if (target === 'refund') {
                setTimeout(LDMLoadRefundData, 100);
            }
        });
    });
});
