...
function _sdmChangeStatus(orderId, newStatus) {
    fetch('/api/admin/order-update-status/', {
        ...
        .then(function (data) {
            if (data.success) {
                window.LDMToast && window.LDMToast.show('Đã cập nhật trạng thái!', 'success');
                _fetchStatDetail();
            } else {
                window.LDMToast && window.LDMToast.show(data.message || 'Lỗi!', 'error');
            }
        })
        .catch(function () { window.LDMToast && window.LDMToast.show('Lỗi kết nối!', 'error'); });
}
...
function doDeleteUser(id) {
    ...
    fetch(window.userDeleteUrl, {
        ...
        .then(data => {
            if (data.success) {
                window.LDMToast.show(data.message, 'success');
                setTimeout(() => location.reload(), 1000);
            } else {
                window.LDMToast.show(data.message, 'error');
            }
        })
        .catch(err => {
            window.LDMToast.show('Có lỗi xảy ra!', 'error');
        });
}
...
function showSidebarPage(page) {
    const menu = document.getElementById('LDMSidebarMenu');
    const paginationEl = document.getElementById('LDMSidebarPagination');
    if (!menu || !paginationEl) return;
    const items = Array.from(menu.querySelectorAll('.ldm-sidebar-item'));
    ...
    let html = '<button type="button" class="ldm-sidebar-prev" ' + (page <= 1 ? 'disabled' : '') + '>Trước</button>';
    for (let p = 1; p <= totalPages; p++) {
        html += '<button type="button" class="ldm-sidebar-page' + (p === page ? ' active-page' : '') + '" data-page="' + p + '">' + p + '</button>';
    }
    html += '<button type="button" class="ldm-sidebar-next" ' + (page >= totalPages ? 'disabled' : '') + '>Sau</button>';
    paginationEl.innerHTML = html;

    paginationEl.querySelector('.ldm-sidebar-prev').onclick = () => showSidebarPage(page - 1);
    paginationEl.querySelector('.ldm-sidebar-next').onclick = () => showSidebarPage(page + 1);
    paginationEl.querySelectorAll('.ldm-sidebar-page').forEach(btn => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const menu = document.getElementById('LDMSidebarMenu');
    const sidebarItems = menu ? menu.querySelectorAll('.ldm-sidebar-item') : [];
...
function deleteColorImageRow(folderId, sku, colorName) {
    if (!folderId || !sku || !colorName) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Thiếu thông tin!', 'error');
        return;
    }

    if (window.LDMConfirm && window.LDMConfirm.show) {
        window.LDMConfirm.show(
...
function performDeleteColorImageRow(folderId, sku, colorName) {
    ...
    fetch('/product-images/color/row-delete/', {
        ...
        .then(data => {
            if (data.success) {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message, 'success');
                loadImageFolderRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể xóa!', 'error');
            }
        })
        .catch(error => {
            console.error('Error deleting color image row:', error);
            window.LDMToast && window.LDMToast.show && window.LDMToast.show('Có lỗi xảy ra!', 'error');
        });
}
...
function deleteImageFolder(id, name) {
    ...
    function doDelete() {
        ...
        fetch('/product-images/folders/delete/', {
            ...
            .then(function (data) {
                if (data.success) {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Đã xóa thư mục!', 'success');
                    loadImageFolderRows();
                } else {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể xóa!', 'error');
                }
            ...
            .catch(function () {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show('Lỗi kết nối!', 'error');
            });
    }
    if (window.LDMConfirm && window.LDMConfirm.show) {
        window.LDMConfirm.show(msg, doDelete);
...
function saveImageFolder() {
    ...
    if (!brandId) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng chọn hãng!', 'error');
        return;
    }

    if (!productId) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng chọn sản phẩm!', 'error');
        return;
    }

    if (!name) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng nhập tên thư mục!', 'error');
        return;
    }
    ...
    fetch('/product-images/folders/create/', {
        ...
        .then(data => {
            if (data.success) {
                if (window.LDMToast && window.LDMToast.show) {
                    window.LDMToast.show(data.message, 'success');
                }
                closeAddImageFolderModal();
                loadImageFolderRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể tạo thư mục.', 'error');
            }
        })
        .catch(error => {
...
function saveColorImageModal() {
    ...
    if (_colorImageEditOriginal && newColor && newColor !== _colorImageEditOriginal.color_name) {
        ...
        fetch('/product-images/color/rename/', {
            ...
            .then(function (data) {
                if (data.success) {
                    window.LDMToast && window.LDMToast.show(data.message, 'success');
                    _colorImageEditOriginal.color_name = newColor;
                } else {
                    window.LDMToast && window.LDMToast.show(data.message || 'Lỗi đổi tên màu!', 'error');
                }
                loadImageFolderRows();
                closeAddColorImageModal();
            })
            .catch(function () {
                window.LDMToast && window.LDMToast.show('Lỗi kết nối!', 'error');
                closeAddColorImageModal();
            });
        return;
    }
    ...
    if (window.LDMToast && window.LDMToast.show) {
        window.LDMToast.show('Đã lưu.', 'success');
    }
    loadImageFolderRows();
    closeAddColorImageModal();
}
...
function uploadColorImage() {
    ...
    if (!folderId || !sku || !colorName || !file) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng chọn thư mục, hãng, SKU, màu và ảnh!', 'error');
        return;
    }
        ...
        .then(data => {
            if (data.success && data.image) {
                ...
                if (window.LDMToast && window.LDMToast.show) {
                    ...
                    window.LDMToast.show(`Đã upload thành công ảnh thứ ${index}.`, 'success');
                }
                // Đặt lại ô chọn file
                ...
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể upload ảnh.', 'error');
            }
        })
        .catch(error => {
    ...
    fetch('/product-images/color/delete/', {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (window.LDMToast && window.LDMToast.show) {
                    window.LDMToast.show(data.message || 'Đã xóa ảnh.', 'success');
                }
                loadImageFolderRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể xóa ảnh.', 'error');
            }
        })
        .catch(error => {
...
function saveEditSku() {
    ...
    if (!newSku) {
        window.LDMToast.show('Vui lòng nhập mã SKU.', 'error');
        return;
    }
    ...
    fetch('/products/sku/edit/', {
        ...
        .then(({ ok, data }) => {
            if (ok && data.success) {
                window.LDMToast.show(data.message || 'Đã sửa SKU.', 'success');
                closeEditSkuModal();
                loadSkuList();
            } else {
                window.LDMToast.show(data.message || 'Không thể sửa SKU.', 'error');
            }
        })
        .catch(error => {
...
function performDeleteSkuItem(id) {
    ...
    fetch('/products/sku/delete/', {
        ...
        .then(({ ok, data }) => {
            if (ok && data.success) {
                window.LDMToast.show(data.message || 'Đã xóa SKU.', 'success');
                loadSkuList();
            } else {
                window.LDMToast.show(data.message || 'Không thể xóa SKU.', 'error');
            }
        })
        .catch(error => {
...
function saveSku() {
    ...
    if (!productId) {
        window.LDMToast.show('Vui lòng chọn sản phẩm!', 'error');
        return;
    }

    if (!sku) {
        window.LDMToast.show('Vui lòng nhập SKU!', 'error');
        return;
    }
...
function uploadBanner() {
    ...
    if (!bannerId) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng nhập ID!', 'error');
        return;
    }

    if (!file) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Vui lòng chọn ảnh!', 'error');
        return;
    }
    ...
    fetch('/banner-images/add/', {
        ...
        .then(data => {
            if (data.success && data.banner) {
                ...
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(`Đã upload thành công ảnh thứ ${index}.`, 'success');

                // Tải lại lưới ảnh
                loadBannerRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể upload banner.', 'error');
            }
        })
        .catch(error => {
...
function quickReplaceBanner(bannerId) {
    ...
    input.addEventListener('change', function () {
        ...
        fetch('/banner-images/replace/', {
            ...
            .then(function (data) {
                if (data.success) {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show('Tải ảnh banner thành công!', 'success');
                    loadBannerRows();
                } else {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể upload banner.', 'error');
                }
            ...
            .catch(function () {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show('Lỗi kết nối server.', 'error');
            })
            .finally(function () {
                input.remove();
            });
    ...
    fetch('/banner-images/add/', {
        ...
        .then(data => {
            if (data.success && data.banner) {
                ...
                window.LDMToast && window.LDMToast.show && window.LDMToast.show('Đã upload thành công.', 'success');
    ...
    fetch('/banner-images/delete/', {
        ...
        .then(data => {
            if (data.success) {
                ...
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Đã xóa banner.', 'success');
                loadBannerRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể xóa banner.', 'error');
            }
        })
        .catch(error => {
...
function saveProductContent() {
    ...
    if (productContentEditorInstance) {
        ...
        productContentEditorPromise.then(editor => {
            ...
            window.LDMToast && window.LDMToast.show && window.LDMToast.show('Không thể lấy nội dung từ editor!', 'error');
        });
    } else {
        ...
        if (editable) {
            ...
            window.LDMToast && window.LDMToast.show && window.LDMToast.show('Editor chưa sẵn sàng!', 'error');
        }
    }
}
...
function saveProductContentData(brandId, productId, contentText) {
    ...
    fetch('/product-content/add/', {
        ...
        .then(data => {
            if (data.success) {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show('Lưu nội dung thành công!', 'success');
                closeAddProductContentModal();
                loadProductContentRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể lưu nội dung.', 'error');
            }
        })
        .catch(error => {
            console.error('Error saving product content:', error);
            window.LDMToast && window.LDMToast.show && window.LDMToast.show(error.message || 'Có lỗi xảy ra!', 'error');
        });
}
...
function quickReplaceProductContent(contentId) {
    ...
    fileInput.onchange = function (e) {
        ...
        fetch('/product-content/replace/', {
            ...
            .then(data => {
                if (data.success) {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show('Tải ảnh thành công!', 'success');
                    loadProductContentRows();
                } else {
                    window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể upload ảnh.', 'error');
                }
...
function openEditProductContentModal(contentId) {
    ...
    if (!content) {
        window.LDMToast && window.LDMToast.show && window.LDMToast.show('Không tìm thấy nội dung!', 'error');
        return;
    }
...
function editProductContent(contentId) {
    ...
    fetch('/product-content/add/', {
        ...
        .then(data => {
            if (data.success) {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show('Cập nhật nội dung thành công!', 'success');
                closeAddProductContentModal();
                loadProductContentRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể lưu nội dung.', 'error');
    ...
    fetch('/product-content/delete/', {
        ...
        .then(data => {
            if (data.success) {
                ...
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Đã xóa nội dung.', 'success');
                loadProductContentRows();
            } else {
                window.LDMToast && window.LDMToast.show && window.LDMToast.show(data.message || 'Không thể xóa nội dung.', 'error');
            }
        })
        .catch(error => {
...
function approveQr(id) {
    ...
    if (!window.LDMConfirm) {
        _doApproveQr(id);
        return;
    }
    window.LDMConfirm.show('Xác nhận duyệt QR chuyển khoản này?', function () {
        _doApproveQr(id);
    });
}
...
function cancelQr(id) {
    ...
    window.LDMConfirm.show('Xác nhận hủy QR chuyển khoản này?', function () {
        _doCancelQr(id);
    });
}
    ...
    fetch(url, {
        ...
        .then(function (data) {
            if (data.success) {
                ...
                window.LDMToast && window.LDMToast.show(msg, 'success');
                closeCouponModal();
                loadCouponList();
            } else {
