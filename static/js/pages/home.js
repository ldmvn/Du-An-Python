...
(function () {
    const slider = document.getElementById('LDMHeroSlider');
    if (!slider) return;

    const slides = Array.from(slider.querySelectorAll('.ldm-hero-slide'));
    const dotsWrap = document.getElementById('LDMHeroDots');
    const dots = dotsWrap ? Array.from(dotsWrap.querySelectorAll('.ldm-hero-dot')) : [];
    const prevBtn = document.getElementById('LDMHeroPrev');
    const nextBtn = document.getElementById('LDMHeroNext');
