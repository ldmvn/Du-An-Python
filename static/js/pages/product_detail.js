...
function addVariantSimple() {
    if (!currentProductId) {
        if (window.LDMToast) {
            window.LDMToast.show('Không xác định được sản phẩm.', 'error');
        } else {
    ...
    if (!storage || !selectedOptions.length || !basePrice) {
        if (window.LDMToast) {
            window.LDMToast.show('Vui lòng nhập đầy đủ: Dung lượng, ít nhất 1 màu (SKU) và giá gốc.', 'error');
        } else {
    ...
    if (!finalPrice || finalPrice <= 0) {
        if (window.LDMToast) {
            window.LDMToast.show('Giá sau giảm không hợp lệ.', 'error');
        } else {
    ...
    Promise.all(requests)
        .then(function (results) {
            ...
            results.forEach(function (data) {
                if (data && data.success) {
                    ...
                    hasError = true;
                    if (window.LDMToast) {
                        window.LDMToast.show(data.message || 'Không thể thêm một số biến thể.', 'error');
                    }
                }
            });
            if (hasSuccess) {
                if (window.LDMToast) {
                    window.LDMToast.show('Đã thêm dung lượng & nhiều màu (SKU).', 'success');
                } else {
        ...
        .catch(function (error) {
            ...
            if (window.LDMToast) {
                window.LDMToast.show('Có lỗi xảy ra khi thêm biến thể!', 'error');
            } else {
        ...
        list.forEach(function (v) {
            ...
            promises.push(
                fetch('/products/variant/save/', { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    ...
                    .then(function (data) {
                        if (!data.success && window.LDMToast) window.LDMToast.show(data.message || 'Lỗi cập nhật biến thể', 'error');
                    })
            );
        });
    });
...
/** Xóa cả nhóm biến thể (1 dung lượng + nhiều màu) — dùng LDMConfirm của hệ thống */
function deleteVariantGroup(btn) {
    ...
    if (window.LDMConfirm && typeof window.LDMConfirm.show === 'function') {
        window.LDMConfirm.show(message, function () {
...
function doDeleteVariantGroup(ids) {
    ...
    Promise.all(ids.map(deleteOne)).then(function () {
        if (window.LDMToast) window.LDMToast.show('Đã xóa nhóm biến thể.', 'success');
        else alert('Đã xóa nhóm biến thể.');
        loadProductDetail(currentProductId);
    }).catch(function (err) {
        console.error(err);
        if (window.LDMToast) window.LDMToast.show('Có lỗi khi xóa.', 'error');
...
function deleteVariant(id, name) {
    ...
    function doDelete() {
        ...
        fetch('/products/variant/delete/', {
            ...
            .then(data => {
                if (data.success) {
                    if (window.LDMToast) window.LDMToast.show(data.message, 'success');
                    else alert(data.message);
                    loadProductDetail(currentProductId);
                } else {
                    if (window.LDMToast) window.LDMToast.show(data.message, 'error');
                    else alert(data.message);
                }
            })
            .catch(error => {
                console.error('Error deleting variant:', error);
                if (window.LDMToast) window.LDMToast.show('Có lỗi xảy ra!', 'error');
                else alert('Có lỗi xảy ra!');
            });
    }
    if (window.LDMConfirm && typeof window.LDMConfirm.show === 'function') {
        window.LDMConfirm.show(message, doDelete);
    } else {
        if (!confirm(message)) return;
        doDelete();
    }
...
function deleteProductImage(imageId) {
    ...
    if (window.LDMConfirm && typeof window.LDMConfirm.show === 'function') {
        window.LDMConfirm.show('Bạn có chắc muốn xóa ảnh này?', doDelete);
    } else {
...
function handleSpecFileChange(event) {
    ...
    if (!file.name.endsWith('.json')) {
        if (window.LDMToast) window.LDMToast.error('Chỉ chấp nhận file .json!');
        event.target.value = '';
        if (fileNameEl) fileNameEl.textContent = '';
        return;
    }
...
function uploadSpecification() {
    ...
    if (!fileInput || !fileInput.files.length) {
        if (window.LDMToast) window.LDMToast.error('Vui lòng chọn file JSON trước!');
        else alert('Vui lòng chọn file JSON trước!');
        return;
    }

    if (!currentDetailId && !currentProductId) {
        if (window.LDMToast) window.LDMToast.error('Không xác định được sản phẩm!');
    ...
    if (!file.name.endsWith('.json')) {
        if (window.LDMToast) window.LDMToast.error('File phải có đuôi .json!');
        return;
    }
    ...
    if (currentDetailId) {
        ...
        if (window.LDMToast) window.LDMToast.error('Vui lòng "Lưu tất cả" trước khi tải thông số!');
        return;
    }
    ...
    fetch(window.specUploadUrl || '/products/specification/upload/', {
        ...
        .then(data => {
            if (data.success) {
                if (window.LDMToast) window.LDMToast.success(data.message || 'Tải file thông số kỹ thuật thành công!');
                ...
                if (window.LDMToast) window.LDMToast.error(data.message || 'Tải file thất bại!');
                else alert(data.message || 'Tải file thất bại!');
            }
        })
        .catch(error => {
            console.error('Spec upload error:', error);
            if (window.LDMToast) window.LDMToast.error('Có lỗi xảy ra khi tải file!');
...
function deleteSpecification() {
    if (!currentHasSpec) {
        if (window.LDMToast) window.LDMToast.error('Chưa có file JSON nào để xóa!');
    ...
    function doDelete() {
        fetch(window.specDeleteUrl || '/products/specification/delete/', {
            ...
            .then(data => {
                if (data.success) {
                    if (window.LDMToast) window.LDMToast.success(data.message || 'Đã xóa thông số kỹ thuật!');
                    else alert(data.message || 'Đã xóa!');
                    loadSpecPreview(null);
                } else {
                    if (window.LDMToast) window.LDMToast.error(data.message || 'Xóa thất bại!');
                    else alert(data.message || 'Xóa thất bại!');
                }
            })
            .catch(error => {
                console.error('Spec delete error:', error);
                if (window.LDMToast) window.LDMToast.error('Có lỗi xảy ra!');
                else alert('Có lỗi xảy ra!');
            });
    }

    if (window.LDMConfirm && typeof window.LDMConfirm.show === 'function') {
        window.LDMConfirm.show('Bạn có chắc muốn xóa thông số kỹ thuật?', doDelete);
    } else {
...
function saveYoutubeId() {
    ...
    fetch('/products/youtube/save/', {
        ...
        .then(data => {
            if (data.success) {
                if (window.LDMToast) window.LDMToast.success(data.message || 'Đã lưu YouTube ID!');
                loadYoutubePreview(data.youtube_id);
            } else {
                if (window.LDMToast) window.LDMToast.error(data.message || 'Lưu thất bại!');
            }
        })
        .catch(error => {
